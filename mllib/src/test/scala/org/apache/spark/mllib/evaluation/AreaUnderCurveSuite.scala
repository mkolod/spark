...
class AreaUnderCurveSuite extends FunSuite with LocalSparkContext {
  test("auc computation") {
    ...
    val auc = 4.0
    assert(AreaUnderCurve.of(curve).almostEquals(auc))
    val rddCurve = sc.parallelize(curve, 2)
    assert(AreaUnderCurve.of(rddCurve)almostEquals(auc))
  }

  test("auc of an empty curve") {
    val curve = Seq.empty[(Double, Double)]
    assert(AreaUnderCurve.of(curve).almostEquals(0.0))
    val rddCurve = sc.parallelize(curve, 2)
    assert(AreaUnderCurve.of(rddCurve).almostEquals(0.0))
  }
